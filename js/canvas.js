...
function Snitch(x, y, img, width, height) {
  this.x = x;
  this.y = y;
  this.img = img;
  this.width = width;
  ...
  this.update = function(enemies) {
    ...
    for (let i = 0; i < enemies.length; i++) {
      ...
      if (distance(player.x, player.y, enemies[i].x, enemies[i].y) - 20 < 0) {
        ...
        // relocate snitch
        snitch.x = randomIntFromRange(
          window.innerWidth + 50 - window.innerWidth,
          window.innerWidth - 50
        );
        snitch.y = randomIntFromRange(
...
function init() {
  ...
  player = new Player(
    mouse.x,
    mouse.y,
    document.getElementById("harry"),
    70,
    60
  );

  // init snitch xy coordinates within canvas boundaries
  ...
  // instantiate snitch
  snitch = new Snitch(sx, sy, document.getElementById("snitch"), 30, 30);
...
function animate() {
  ...
  // listen for collision with snitch and avoid collision on new frame
  if (distance(player.x, player.y, snitch.x, snitch.y) < 25) {
